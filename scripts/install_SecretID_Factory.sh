...
register_secret_id_service_with_consul () {
    ...
    {
      "Name": "secretidfactory",
      "Tags": [
        "approle"
      ],
      "Port": 8314,
      "Meta": {
        "Secret-ID-Factory-Service": "0.0.1"
      },
      "EnableTagOverride": false,
      "checks": [
          {
            "id": "api-http",
            "name": "Factory Service Secret ID HTTP Check",
          ...
          {
            "id": "api-tcp",
            "name": "Factory Service Secret ID TCP Check",
            "tcp": "127.0.0.1:8314",
            "interval": "10s",
            "timeout": "30s"
          }
...
start_app_proxy_service () {
  ...
  create_service "${1}" "${2}" "/usr/local/bin/consul connect proxy -http-addr=https://127.0.0.1:8321 -ca-file=/usr/local/bootstrap/certificate-config/consul-ca.pem -client-cert=/usr/local/bootstrap/certificate-config/cli.pem -client-key=/usr/local/bootstrap/certificate-config/cli-key.pem -token=${AGENTTOKEN} -sidecar-for ${3}"
...
start_app_envoy_proxy_service () {
  # start the new service mesh proxy for the application
  # param 1 ${1}: app-proxy name
  # param 2 ${2}: app-proxy service description

  create_service "${1}" "${2}" "/usr/local/bin/consul connect envoy -http-addr=https://127.0.0.1:8321 -ca-file=/usr/local/bootstrap/certificate-config/consul-ca.pem -client-cert=/usr/local/bootstrap/certificate-config/cli.pem -client-key=/usr/local/bootstrap/certificate-config/cli-key.pem -token=${AGENTTOKEN} -sidecar-for ${3}"
  sudo usermod -a -G consulcerts ${1}
  sudo systemctl start ${1}
  sudo systemctl status ${1}
  echo "${1} Proxy App Service Build Complete"
...
start_client_envoy_proxy_service () {
    # start the new service mesh proxy for the client
    # param 1 ${1}: client-proxy name
    # param 2 ${2}: client-proxy service description
    # param 3 ${3}: client-proxy upstream consul service name
    ...
    create_service "${1}" "${2}" "/usr/local/bin/consul connect envoy -http-addr=https://127.0.0.1:8321 -ca-file=/usr/local/bootstrap/certificate-config/consul-ca.pem -client-cert=/usr/local/bootstrap/certificate-config/cli.pem -client-key=/usr/local/bootstrap/certificate-config/cli-key.pem -token=${AGENTTOKEN} -service ${1} -upstream ${3}:${4}"
    sudo usermod -a -G consulcerts ${1}
    sudo systemctl start ${1}
    sudo systemctl status ${1}
    echo "${1} Proxy Client Service Build Complete"
...
setup_environment () {
    ...
    IFACE=`route -n | awk '$1 == "192.168.99.0" {print $8}'`
...
install_go_application () {
    ...
    if [ ! "${TRAVIS}" == "true" ]; then
        create_service factory "SecretID Factory Service (approle)" "/usr/local/bin/VaultServiceIDFactory -ip=127.0.0.1 -vault=\"${VAULT_ADDR}\""
    ...
    sleep 15
    start_app_envoy_proxy_service secretidproxy "Proxy for SecretID Factory Service (using approle)" secretidfactory
    sleep 15
